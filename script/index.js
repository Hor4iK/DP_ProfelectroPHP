...
const replaceToCart = () => {
  setTimeout(() => {
    window.location.replace("/content/cart.php");
  }, 300);
};
...
function setHandlersButtonsSubmit() {
  const buttonsSubmit = document.querySelectorAll(
    ".content__catalog__card-button",
  );
...
const handlerCallDeleteGood = (evt) => {
  ...
  Promise.all([callFunctionAllocator("deleteGoodFromCart", args)])
    .then(() => {
      callFunctionAllocator("getCart").then((data) => {
...
const selectCardsForDelete = (evt) => {
  ...
  const checkedItemList = cardArray.filter(
    (item) => item.querySelector(".data__list-item-check").checked,
  );
...
const handlerCallDeleteBanners = (evt) => {
  ...
  callFunctionAllocator("deleteBanner", args)
    ...
    .then(
      generateMessage(
        messageTitle.success,
        messageText.successRemoveBanners,
        "success",
      ),
    );
};
function setHandlerDeleteBannersButton() {
  const deleteButton = document.querySelector(
    ".data__actions__btn_delete_banners",
  );
...
const createCardBanner = (template, cardData) => {
  ...
  cardData.banner_image
    ? (image.src = cardData.banner_image)
    : (image.src = "../img/default-product-image.png");
    ...
    if (!hasInvalidInput(inputList)) {
      ...
      callFunctionAllocator("addBanner", args)
        .then((res) => {
          generateMessage(
            messageTitle.success,
            messageText.successAddBanner,
            "success",
          );
        })
        .then(closeModal(modal))
        .then(() => {
          callFunctionAllocator("getBanners").then((data) => {
            const res = Array.from(data.response);
            container.querySelectorAll(".card").forEach((item) => {
              item.remove();
            });
            res.forEach((item) => {
              container.append(createCardBanner(template, item));
            });
          });
        })
        .catch((err) => {
          generateMessage(messageTitle.error, messageText.errorAdd, "danger");
        });
    } else {
      // evt.preventDefault();
    }
...
const handlerCallDeleteOffers = (evt) => {
  ...
  callFunctionAllocator("deleteOffers", args)
    ...
    .then(
      generateMessage(
        messageTitle.success,
        messageText.successChangesOffers,
        "success",
      ),
    );
};
function setHandlerDeleteOffersButton() {
  const deleteButton = document.querySelector(
    ".data__actions__btn_delete_offers",
  );
...
const setHandlerAddOffersButton = (modal, container, template) => {
  ...
  const buttonAdd = modal.querySelector(".popup__button");
  const handlerCard = (template, item) => {
    const card = createAdminPanelCard(template, item);
    const input = card.querySelector(".input-card");
    if (item.offer_good_turn == 1) input.checked = true;
    return card;
  };
  buttonAdd.addEventListener("click", (evt) => {
    const checkedList = cardsList
      .map((item) => {
        if (item.querySelector(".popup__input").checked) return item;
      })
    ...
    if (checkedList.length != 0) {
      ...
      callFunctionAllocator("addOffers", args)
        .then((data) => {
          ...
          res.forEach((item) => {
            container.append(handlerCard(template, item));
          });
          generateMessage(
            messageTitle.success,
            messageText.successChangesOffers,
...
const handlerCallDeleteGoodDB = (evt) => {
  ...
  callFunctionAllocator("deleteGood", args)
    ...
    .then(
      generateMessage(
        messageTitle.success,
        messageText.successRemoveGoods,
        "success",
      ),
    );
};
function setHandlerDeleteGoodsButton() {
  const deleteButton = document.querySelector(
    ".data__actions__btn_delete_goods",
  );
...
const setHandlersListenersInput = () => {
  ...
  cardsList.forEach((item) => {
    ...
    input.addEventListener("input", () => {
      ...
      Promise.all([callFunctionAllocator("changeGoodCart", args)]).then(() => {
        callFunctionAllocator("getCart").then((data) => {
          const dataArray = Array.from(data.response);
          dataArray.forEach((item) => {
...
function setHandlerInputSearch(searchConfig, loader) {
  searchConfig.input.addEventListener("input", () => {
    ...
    searchConfig.containerSearch.querySelectorAll(".card").forEach((item) => {
      item.remove();
    });
    if (
      searchConfig.input.value != "" &&
      searchConfig.input.value != null &&
      searchConfig.input.value != " "
    ) {
      if (!searchConfig.input.validity.patternMismatch) {
        ...
        callFunctionAllocator(searchConfig.method, args)
          .then((data) => {
            ...
            if (res[0]) {
              if (res.length > 1)
                searchConfig.titlePage.textContent = `Найдено: ${res.length} товаров`;
              else if ((res.length = 1))
                searchConfig.titlePage.textContent = `Найден: ${res.length} товар`;
              else if (res.length > 1 && res.length < 5)
                searchConfig.titlePage.textContent = `Найдено: ${res.length} товара`;
              if (searchConfig.admin) {
                res.forEach((item) => {
                  searchConfig.containerSearch.append(
                    createAdminPanelCard(searchConfig.templateCard, item),
                  );
                });
              } else {
                searchConfig.container.classList.add(
                  "content__catalog-container_type_hide",
                );
                res.forEach((item) => {
                  searchConfig.containerSearch.append(
                    createCard(
                      searchConfig.templateCard,
                      item,
                      searchConfig.popupConfig,
                      searchConfig.allPriceList,
                    ),
                  );
                });
              }
      ...
      if (searchConfig.admin) {
        ...
        searchConfig.container.classList.remove(
          "content__catalog-container_type_hide",
        );
      }
    }
